...
By default, the `latest` version is installed; or the versions listed in the [`target`](./config-file.mdx#target) array in the configuration file, if the option is set; or the versions specified via the [`--target`](#target) command line option, if it is provided.
...
You can target minor `x.y` or patch `x.y.z` releases. The `x.y` tag format resolves to the latest version in that serries (e.g. passing `4.9` runs type tests on TypeScript 4.9.5).
...
# Run tests using the currently installed version of TypeScript
tstyche --target current

# Test on all specified versions of TypeScript
tstyche --target 4.7,5.0.4,latest
```
