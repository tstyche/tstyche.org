...
You can target minor `"x.y"` or patch `"x.y.z"` releases. The `"x.y"` tag resolves to the latest version in the serries (e.g. passing `"4.9"` resolves to version 4.9.5). The `"beta"`, `"latest"`, `"next"` and `"rc"` tags are supported.
