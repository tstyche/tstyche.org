...
To be treated as a directive, the comment text must start with `@tstyche` namespace followed by the directive keyword. Arguments must be provided if a directive requires them. Optionally, a note can be added after two or more hyphens.
