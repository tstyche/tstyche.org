...
From time to time [the store](../guide/typescript-versions.mdx#the-store) fetches the latest metadata from the registry. This makes sure that tags like `latest` or `next` resolve correctly.
