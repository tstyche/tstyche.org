...
This can be a JavaScript library which ships with hand crafted type definitions. To make testing the definitions a breeze, TSTyche sets sensible default compiler options and provides the `typescript` package.
...
To specify other version of TypeScript to test on, pass the [`--target`](../reference/command-line.mdx#target) command line option or use the [`target`](../reference/config-file.mdx#target) configuration option.
