...
**Signature.** The modified value must be returned as a an object:

```ts
config(resolvedConfig: ResolvedConfig) => ResolvedConfig
```
...
**Signature.** The modified value must be returned as a list of strings, URL strings or URL objects:

```ts
select(testFiles: Array<string>) => Array<string | URL>
```
