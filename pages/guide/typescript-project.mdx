...
TSTyche is build to test projects of any size and complexity. To retrieve the type information it wraps around TypeScript's [language service](https://github.com/microsoft/TypeScript/wiki/Using-the-Language-Service-API). The same API which is used by code editors like Visual Studio Code via the [`tsserver`](https://github.com/microsoft/TypeScript-wiki/blob/main/Standalone-Server-(tsserver).md) executable.

## TSConfig File

For each test file the service loads the compiler configuration from the nearest `tsconfig.json` which includes the file in question. This means that TSTyche sees exactly the same types which you see in hover messages.

If a particular test file is not included in any of TSConfig files found in enclosing directories, the [default compiler options](./javascript-project.mdx#default-compiler-options) will be set.
