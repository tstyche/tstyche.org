...
TSTyche is build to test projects of any size and complexity. To retrieve the type information it wraps around TypeScript's `ProjectService`. The same API which is used by code editors like Visual Studio Code via the [`tsserver`](https://github.com/microsoft/TypeScript-wiki/blob/main/Standalone-Server-(tsserver).md) executable.
