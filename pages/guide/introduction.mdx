...
Take a look at any popular JavaScript test runner. It will be a command line tool. It will have `test()` and `describe()` helpers with `.only` and `.skip` modifiers. It will be able to select test files using a pattern or to run a specific test file. And it will report success in green and failures in red.
