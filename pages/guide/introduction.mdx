...
This guide recommends using the latest Long Term Support (LTS) release of Node.js (the minimum required version is `16.14`). Also the internet connection must be available. To learn more, see the [System Requirements](../reference/requirements.mdx) page.
...
[31mError: [0mType '"getLength"' is not identical to type '"getLength" | "getWidth"'.

  [90m 8 | }[0m
  [90m 9 | [0m
[31m>[0m 10 [90m|[0m expect<MethodLikeKeys<Sample>>().type.toBe<"getLength" | "getWidth">();
     [90m|[0m                                       [31m^[0m
  [90m11 | [0m
