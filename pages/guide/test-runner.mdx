...
This page assumes you already know how to install TSTyche and run type tests. If not, head to the [Get Started](./get-started.mdx) page first.
...
By default, the currently installed `typescript` package is used. To specify other version of TypeScript, pass it with the [`--target`](../reference/command-line.mdx#target) command line option (or use the [`target`](../reference/config-file.mdx#target) configuration option):

<Tabs items={["npm", "yarn", "pnpm"]} storageKey={"preferredPackageManager"}>
<Tabs.Tab>

```shell
npx tstyche --target '>=5.6'
```
...
```shell
pnpm tstyche --target '>=5.6'
```

</Tabs.Tab>
</Tabs>

If you run this command using the example from the [Get Started](./get-started#define-a-type) page, the report should include three versions of TypeScript:

```ansi
[34muses[0m TypeScript 5.6.3

[32mpass[0m [90m./[0mMethodLikeKeys.tst.ts

[34muses[0m TypeScript 5.7.3
...
At this point you can rest assured that typings can be used with TypeScript `5.6` and above. Wondering about the future? Specify `beta`, `latest`, `next` or `rc` as the target tag.
...
- [`--install`](../reference/command-line.mdx#install) fetches the specified versions of TypeScript, useful if you are preparing to work offline,
