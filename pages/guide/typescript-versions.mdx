...
The projects that declare compatibility with TypeScript, mostly choose to specify a single version or a range of versions.
...
The directive can conditionally skip an `expect()`, `test()`, `describe()` or the whole test file (when specified at the top of a file).
