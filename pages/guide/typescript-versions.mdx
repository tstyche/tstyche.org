...
Keep in mind that TypeScript follows the semver policy somewhat [loosely](https://github.com/microsoft/TypeScript/wiki/TypeScript's-Release-Process/e669ab1ad96edc1a7bcef5f6d9e35e24397891e5). A patch release is strictly only a bug fix, but a major release may introduce breaking changes. This means, upgrading from `5.6` to `5.7` can beak your types.
...
For example, it is a good idea to have a workflow that runs tests once a day against `typescript@next`. An example can be found in [`tstyche/ts-nightly-example`](https://github.com/tstyche/ts-nightly-example) repository.
