...
Keep in mind that TypeScript follows the Semantic Versioning specification somewhat [loosely](https://github.com/microsoft/TypeScript/wiki/TypeScript's-Release-Process/e669ab1ad96edc1a7bcef5f6d9e35e24397891e5). A patch release is still strictly only a bug fix, but a major release may introduce breaking changes. This means upgrading from `5.6` to `5.7` can break your types.

If your project declares that the lowest supported TypeScript version is `5.6`, it is recommended to test against all major versions starting with that one:
