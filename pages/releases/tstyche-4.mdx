...
To skip an `expect()`, `test()` or `describe()` on specific version of TypeScript, use the `// @tstyche if { target: <range> }` comment directive:
...
Adding the directive at the top of the file, would skip the whole file when condition does not match.
