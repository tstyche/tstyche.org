...
In some cases it is useful to generate tests using a template and data table. To cover this use case TSTyche 4 is adding support for template test files.

To mark a test file as a template, a user must include `// @tstyche-template` as the first line of a test file.

When the `// @tstyche-template` directive is detected, the default export of a file is treated as a type test text.
