...
The logic of glob pattern matching is reworked. All matching paths in a directory are selected, because of that file extensions must be always specified. For example, to avoid selecting the `tsconfig.json` file within the `__tests__` directory:
