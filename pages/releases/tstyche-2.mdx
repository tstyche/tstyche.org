...
Testing on specific versions of TypeScript requires Node.js 18 or later. The global [`fetch()`](https://developer.mozilla.org/en-US/docs/Web/API/fetch) method is now used to fetch the `typescript` package metadata from the registry.

#### `--watch` [#watch]

The watch mode is added. Current implementation is watching the TSTyche configuration file and the test files for changes and reruns them.
