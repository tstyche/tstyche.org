...
TSTyche 3.0 ships with support of a custom `npm` registry, the new `omit()` and `pick()` utilities, support for reporters and plugins, programmatic usage refinements and performance improvements.
