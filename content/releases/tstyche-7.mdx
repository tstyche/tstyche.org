...
## TSTyche 7.2  [#tstyche-7-2]

TSTyche 7.2 ships with reimplemented `.toAcceptProps()` matcher and adds fetch retry logic.

### Matchers

#### `.toAcceptProps()` [#to-accept-props]

The `.toAcceptProps()` matcher logic is reimplemented to use the ability layer. Instead of manually walking type properties, the matcher now transforms `expect(Button).type.toAcceptProps({ text: "Send" })` into a synthetic JSX element `<Button text={"Send"}/>` and lets TypeScript's type checker to produce the diagnostics. The change unblocks generic component support.

### Environment Variables

#### `TSTYCHE_FETCH_RETRIES` [#fetch-retries]

The number of times to retry a failed HTTP request.

#### `TSTYCHE_FETCH_TIMEOUT` [#fetch-timeout]

The maximum number of seconds to wait before an HTTP request times out. The value of `0` disables the timeout.
