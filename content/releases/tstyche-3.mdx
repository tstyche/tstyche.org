...
For instance, the following test is passing even when the return type accidentally resolves to to `any`:
...
For instance, the following test is passing even when the `Result` type accidentally resolves to to `never`:
