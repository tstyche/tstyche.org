...
TSTyche 6.0 is available as a release candidate. It is production-ready and no further breaking changes will be introduced before the stable version. To install the latest beta version, use the `next` distribution tag:
...
Previously ignored, `this` parameter, `const` type parameter modifier or `NoInfer` marker are now taken into account:
