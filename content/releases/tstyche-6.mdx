...
## TSTyche 6.2  [#tstyche-6-2]

TSTyche 6.2 ships with the new `dot` reporter and the `--verbose` command line option.
...
Thanks to [@voxpelli](https://github.com/voxpelli) for the idea!
...
Previously ignored, `this` parameters, `const` type parameters and `NoInfer` markers are now taken into account:
...
**Breaking!** Support for plugins is removed. Most use cases should be covered by the [comment directives](../reference/directives.mdx).
