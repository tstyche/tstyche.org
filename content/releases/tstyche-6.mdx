...
The new implementation brings in several improvements to the matcher. It considers the following types as identical:

- `{ a: string } & { b: number }` and `{ a: string; b: number }`
- `((a: string) => string) & ((b: number) => number)` and `{ (a: string): string; (b: number): number }`

Previously ignored, the `this` parameter and `const` type parameter modifier are now taken into account:

- `(this: unknown) => void` is not the same as `() => void`
- `<const T>(arg: T) => T` is not the same as `<T>(arg: T) => T`
