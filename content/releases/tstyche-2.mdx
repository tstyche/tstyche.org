...
TSTyche 2.1 adds the new `.toAcceptProps()` matcher.
...
Testing on specific version of TypeScript requires Node.js 18 or later. The global `fetch()` method is now used to fetch the `typescript` package metadata from the registry.

### Configuration

#### `testFileMatch` [#test-file-match]

The logic of glob pattern matching is reworked. The implementation now allows selecting all matching paths including `.json` or `.mdx` files.
...
Watch mode is added. TSTyche watches the configuration file and the test files for changes.
...
The new name makes it easier to switch between primitive type matchers and `.toBe()`. For example, replacing `.toBeString()` with `.toBe<string | undefined>()` looks more clear than before.
