...
<Tabs defaultIndex={1} selectedIndex={1} items={["aube", "npm", "pnpm", "yarn"]} storageKey={"packageManager"}>
<Tabs.Tab>
...
By convention, unit tests are suffixed with `.test.*` and type tests with `.tst.*`. To learn more, see the [File Structure](../project/file-structure.mdx) page.
...
TSTyche does not execute the code. Instead, it analyzes types programmatically and reports the test run results. To learn more, see the [Introduction](./introduction.mdx) page.
