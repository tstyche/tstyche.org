...
TSTyche searches for the nearest `tsconfig.json` that includes the test file. If none is found, it uses default compiler options.

## TSConfig File

To load the compiler options for a test file, TSTyche searches for the nearest `tsconfig.json` which includes the file in question. If a particular test file is not included in any of the TSConfig files found in enclosing directories, [default compiler options](#default-compiler-options) are set.
