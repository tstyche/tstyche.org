...
Although both of these tools do report errors, their tasks do not include testing types. In other words, neither of them has features that a test runner has. For instance, the compiler does not need an error message like: `Type 'A' is not the same as type 'B'.`
...
To be able to test types TSTyche loads the `typescript` package installed in your project. It checks types programmatically and does not require any dependencies to be installed. This is why TSTyche has a minimal install size of around 300kB.
