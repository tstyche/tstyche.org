import { Note, Tip, Warning } from "@/components/Advice";

# Expect API

TSTyche ships `expect` style assertions to check types.

<Note>

TSTyche uses TypeScript API to infer types of expressions. For an overview of the aspects of TypeScript type inference that commonly affect type tests, see the [Type Inference](../explanations/type-inference.mdx) page.

</Note>

## `expect()`

The `expect()` function builds an assertion. It must be followed by the `.type` modifier and a [matcher](#matchers). Optionally, a [run mode flag](#run-mode-flags) can be appended. `expect()` can be nested inside other expressions.

---

**Signatures.** The source type must be provided either as a type argument or inferred from an expression.

```ts
expect<S>()
expect(source: unknown)
```

## Run Mode Flags

To focus or skip assertions, append the run mode flags to the `expect()` function. The flags are inherited from the parent.

### `.only`

Marks an assertion as focused. When there are focused groups, tests or assertions in a file, only those will run.

### `.skip`

Marks an assertion as skipped. The test runner will ignore anything skipped and will suppress type errors inside the skipped assertion.

<Tip>

Use the `// @tstyche fixme` comment directive to mark any `expect()` as failing. To learn more, see the [Comment Directives](./directives.mdx#fixme) page.

</Tip>

## Matchers

A matcher is the final element in the assertion chain. It defines the condition to check.

### `.not` [#not]

To negate a matcher, prepend `.not` before it:

```ts
import { expect } from "tstyche";

expect<string>().type.toBe<string>();
expect<number>().type.not.toBe<string>();
```

### `.toAcceptProps()` [#to-accept-props]

Checks if the JSX component accepts the given props.

```tsx twoslash
// @jsx: react-jsx
import type { ReactNode } from "react";
import { expect, test } from "tstyche";

interface CardProps {
  title: string;
  children: ReactNode;
}

function Card({ title, children }: CardProps) {
  return (
    <>
      <h2>{title}</h2>
      {children}
    </>
  );
}

test("Card", () => {
  expect(Card).type.toAcceptProps({ title: "Hello", children: "Click me" });
  expect(Card).type.toAcceptProps({ title: "Hello", children: <span>Click me</span> });

  expect(Card).type.not.toAcceptProps({ title: "Hello" });
  expect(Card).type.not.toAcceptProps({});
});
```

The props object is interpreted as JSX syntax:

- `{ text: "Send", active: false }`, becomes `<Component text={"Send"} active={false} />`
- `{ "data-testid": "submit-btn" }`, becomes `<Component data-testid={"submit-btn"} />`
- `{ ...userData }`, becomes `<Component {...userData} />`

Spread props do not undergo [excess property checks](../explanations/type-inference.mdx#excess-property-checks).

<Note>

The matcher can only be used in `.tsx` files and the [`jsx`](https://www.typescriptlang.org/tsconfig/#jsx) compiler option must be configured.

</Note>

---

**Signature.** The props must be provided as an object literal.

```ts
.toAcceptProps(props: Record<string, unknown>)
```

### `.toBe()` [#to-be]

Checks if the source type is the same as the target type.

```ts twoslash
import { expect } from "tstyche";

type AsyncProps<T> = {
  [K in keyof T]+?: T[K] | Promise<T[K]>;
};

type WithLoading<T> = T & { loading: boolean };

expect<WithLoading<AsyncProps<{ query: string }>>>().type.toBe<{
  query?: string | Promise<string>;
  loading: boolean;
}>();
```

The types are compared structurally. As shown in the example above, the matcher treats `{ a: string } & { b: number }` and `{ a: string; b: number }` as equivalent.

The matcher allows granular checks of every detail. For example, it distinguishes between types with and without the `NoInfer` marker:

```ts twoslash
import { expect } from "tstyche";

const infersLiteralType = <T>(value: T) => value;
const blocksTypeInference = <T>(value: NoInfer<T>) => value;

expect(infersLiteralType("kiwi")).type.toBe<"kiwi">();
expect(blocksTypeInference("kiwi")).type.toBe<unknown>();

// Identical input results in a different return type,
// therefore the matcher treats these signatures as non-equivalent
expect(infersLiteralType).type.not.toBe(blocksTypeInference);
```

---

**Signatures.** The target type must be provided either as a type argument or inferred from an expression.

```ts
.toBe<T>()
.toBe(target: unknown)
```

### `.toBeApplicable` [#to-be-applicable]

Checks if the decorator function can be applied to a class or a class member.

Unlike other matchers, `.toBeApplicable` must be used as a decorator:

```ts twoslash
import { expect, test } from "tstyche";

function bind<This, Value extends (this: This, ...args: any) => any>(
  target: Value,
  context: ClassMethodDecoratorContext<This, Value>,
) {
  // ...
}

test("bind", () => {
  class Fixture {
    #name: string;

    constructor(name: string) {
      this.#name = name;
    }

    @(expect(bind).type.toBeApplicable) // <-- used as a decorator here
    toString() {
      return `Hello, my name is ${this.#name}.`;
    }
  }
});
```

### `.toBeAssignableFrom()` [#to-be-assignable-from]

Checks if the source type is assignable from the target type.

```ts twoslash
import { expect, test } from "tstyche";

type Awaitable<T> = T | PromiseLike<T>;

test("Awaitable", () => {
  expect<Awaitable<string>>().type.toBeAssignableFrom("abc");
  expect<Awaitable<string>>().type.toBeAssignableFrom(Promise.resolve("abc"));

  expect<Awaitable<string>>().type.not.toBeAssignableFrom(123);
  expect<Awaitable<string>>().type.not.toBeAssignableFrom(Promise.resolve(123));
});
```

<Tip>

Use the [`.toBeAssignableTo()`](#to-be-assignable-to) matcher to test the relationship in the reverse direction.

As a best practice, always keep the type under test on the left hand side of the assertion. This makes the test easier to read:

```diff
- expect<Set<string>>.type.toBeAssignableFrom(new Set(["abc"]));
+ expect(new Set(["abc"])).type.toBeAssignableTo<Set<string>>();
```

</Tip>

---

**Signatures.** The target type must be provided either as a type argument or inferred from an expression.

```ts
.toBeAssignableFrom<T>()
.toBeAssignableFrom(target: unknown)
```

### `.toBeAssignableTo()` [#to-be-assignable-to]

Checks if the source type is assignable to the target type.

```ts twoslash
import { expect, test } from "tstyche";

test("Set", () => {
  expect(new Set(["abc"])).type.toBeAssignableTo<Set<string>>();
  expect(new Set([123])).type.toBeAssignableTo<Set<number>>();

  expect(new Set([123, "abc"])).type.not.toBeAssignableTo<Set<string>>();
  expect(new Set([123, "abc"])).type.not.toBeAssignableTo<Set<number>>();
});
```

<Tip>

Use the [`.toBeAssignableFrom()`](#to-be-assignable-from) matcher to test the relationship in the reverse direction.

As a best practice, always keep the type under test on the left hand side of the assertion. This makes the test easier to read:

```diff
- expect("abc").type.toBeAssignableTo<Awaitable<string>>();
+ expect<Awaitable<string>>().type.toBeAssignableFrom("abc");
```

</Tip>

---

**Signatures.** The target type must be provided either as a type argument or inferred from an expression.

```ts
.toBeAssignableTo<T>()
.toBeAssignableTo(target: unknown)
```

### `.toBeCallableWith()` [#to-be-callable-with]

Checks if a function is callable with the given arguments.

```ts twoslash
import { expect, test } from "tstyche";

function isSameLength<T extends { length: number }>(a: T, b: T) {
  return a.length === b.length;
}

test("isSameLength", () => {
  expect(isSameLength([1, 2], [1, 2, 3])).type.toBe<boolean>();
  expect(isSameLength("one", "two")).type.toBe<boolean>();

  expect(isSameLength).type.not.toBeCallableWith(1, 2);
  expect(isSameLength).type.not.toBeCallableWith("zero", [123]);

  expect(isSameLength<string | Array<number>>).type.toBeCallableWith("zero", [123]);
});
```

---

**Signature.** The matcher accepts zero or more arguments.

```ts
.toBeCallableWith(...args: Array<unknown>)
```

### `.toBeConstructableWith()` [#to-be-constructable-with]

Checks if a class is constructable with the given arguments.

```ts twoslash
import { expect, test } from "tstyche";

class Pair<T> {
  left: T;
  right: T;

  constructor(left: T, right: T) {
    this.left = left;
    this.right = right;
  }
}

test("Pair", () => {
  expect(Pair).type.toBeConstructableWith("sun", "moon");
  expect(Pair).type.toBeConstructableWith(true, false);

  expect(Pair).type.not.toBeConstructableWith("five", 10);
  expect(Pair<number | string>).type.toBeConstructableWith("five", 10);

  expect(Pair).type.not.toBeConstructableWith();
  expect(Pair).type.not.toBeConstructableWith("nope");
});
```

---

**Signature.** The matcher accepts zero or more arguments.

```ts
.toBeConstructableWith(...args: Array<unknown>)
```

### `.toBeInstantiableWith()` [#to-be-instantiable-with]

Checks if a generic is instantiable with the given type arguments.

```ts twoslash
import { type _, expect, test } from "tstyche";

interface Matchers<R, T = unknown> {
  [key: string]: (expected: T) => R;
}

test("Matchers", () => {
  expect<Matchers<_>>().type.toBeInstantiableWith<[void]>();
  expect<Matchers<_>>().type.toBeInstantiableWith<[void, string]>();

  expect<Matchers<_>>().type.not.toBeInstantiableWith<[]>();
});
```

<Tip>

The `_` type can be used to fill in the required type arguments.

</Tip>

This matcher works with generic interfaces, type aliases, classes and functions:

```ts twoslash
import { expect, test } from "tstyche";

function map<T, U>(array: Array<T>, transform: (item: T) => U): Array<U> {
  return array.map(transform);
}

test("map", () => {
  expect(map).type.toBeInstantiableWith<[string, number]>();
  expect(map).type.not.toBeInstantiableWith<[string]>();
})
```

---

**Signature.** The type arguments must be provided as a tuple type.

```ts
.toBeInstantiableWith<T extends [...args: Array<unknown>]>();
```

### `.toHaveProperty()` [#to-have-property]

Checks if a property key exists on the source type.

```ts twoslash
import { expect, test } from "tstyche";

type Worker<T> = {
  [K in keyof T as Exclude<K, "setup" | "teardown">]: T[K];
};

interface Sample {
  [key: `data-${string}`]: string;
  isBusy?: boolean | undefined;
  runTest: (a: string, b: number) => void;
  setup: () => void;
  teardown: () => void;
}

test("Worker", () => {
  expect<Worker<Sample>>().type.toHaveProperty("data-sample");
  expect<Worker<Sample>>().type.toHaveProperty("isBusy");
  expect<Worker<Sample>>().type.toHaveProperty("runTest");

  expect<Worker<Sample>>().type.not.toHaveProperty("setup");
  expect<Worker<Sample>>().type.not.toHaveProperty("teardown");
});
```

---

**Signature.** The `key` argument must be a string, number or symbol.

```ts
.toHaveProperty(key: string | number | symbol)
```

### `.toRaiseError()` [#to-raise-error]

Checks if the type raises an error.

<Warning>

The `.toRaiseError()` matcher is deprecated and planned to be removed. The suggested replacement is the ability matchers (like [`.toBeCallableWith()`](../reference/expect-api.mdx#to-be-callable-with) or [`.toBeConstructableWith()`](../reference/expect-api.mdx#to-be-constructable-with)). They communicate the intent of a test more clearly and work without introducing type errors in the test code.

To learn more, see the [Expect Errors](../guides/expect-errors.mdx) page.

</Warning>
