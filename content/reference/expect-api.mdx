...
TSTyche is able to check error messages suppressed by `@ts-expect-error` directives in the test files. It is recommended to use `@ts-expect-error` instead of `.toRaiseError()`. To learn more, see the documentation of the [`checkSuppressedErrors`](./config-file.mdx#check-suppressed-errors) option.
