...
By default, the configuration file is loaded from the current directory. When the [`--root`](./command-line.mdx#root) command line option is set, the configuration file is searched for in that directory.

Use the [`--config`](./command-line.mdx#config) command line option to point to a specific configuration file explicitly.
...
For instance, the following test is passing even when the return type accidentally resolves to `any`:
...
This issue can be solved by adding `.not.toBe<any>()` check. `rejectAnyType` does the same, but without the boilerplate.
...
For instance, the following test is passing even when the `Result` type accidentally resolves to `never`:
...
This issue can be solved by adding `.not.toBe<never>()` check. `rejectNeverType` does the same, but without the boilerplate.
...
- automatically discover the nearest `tsconfig.json` file (default),
- use default compiler options,
- load a specific `tsconfig.json` file,
- provide the TSConfig as an inline JSON string.

<Warning>

When TSConfig file does not include the test file in question, [default compiler options](../project/compiler-options.mdx#default-compiler-options) will be set.
