...
To configure TSTyche add a `tstyche.config.json` file to the project.
...
```json
{
  "$schema": "https://tstyche.org/schemas/config-5.0.json"
}
```
...
- Default: the current directory; or the path to the directory from which the configuration file was loaded
- Type: `string`

The path to a directory containing files of a test project.

To collect the test files, the file tree is walked starting from the the root directory. Later during the test run, the lookup of `tsconfig.json` for each test file starts at the directory containing the file and ends at the the root directory.

This means that all test files to be run and TSConfig files to load the compiler configuration from must be within the the root directory.
...
- or, if a path is specified, load `tsconfig.json` from that path.
