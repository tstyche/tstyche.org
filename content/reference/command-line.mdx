...
- **list** can hold one or several values separated by commas: `--reporters 'list, summary'`.
...
When the `--root` command line option is set, the configuration file is searched for in that directory. To avoid this, set an explicit [`--config`](#config) path as well:
